...
| 문서 버전 | v1.6 |
| 작성일 | 2026-08-07 |
| 상태 | 초안 — 3차 재검수(RT-11·RT-12) 반영: 2FA 강제 기준의 데이터 파생화(`requires_2fa`), 온보딩 게이트 명세 (변경 내역은 §17) |
...
- `OPERATOR`·`SUPER_ADMIN`은 `requires_2fa=true`로 시드한다(§10.4). 신규 역할 생성 시 이 플래그는 기본 false이며 명시적으로 선택한다.
...
CREATE TABLE users (
  ...
  must_change_password     BOOLEAN NOT NULL DEFAULT false, -- 온보딩 게이트 (§8.5)
  totp_enrollment_required BOOLEAN NOT NULL DEFAULT false, -- 온보딩 게이트 (§8.5)
  totp_secret    VARCHAR(255),                   -- TOTP 시크릿. 애플리케이션 계층 암호화 보관, API 응답 노출 금지(§10.2)
  created_at     TIMESTAMPTZ  NOT NULL DEFAULT now(),
...
CREATE TABLE roles (
  ...
  requires_2fa BOOLEAN     NOT NULL DEFAULT false, -- 보유 계정에 2FA(TOTP) 강제 (§10.4)
...
- [ADM-2] 역할 생성/수정/삭제 — `admin.role.manage` | 역할의 권한 파워 통제는 ADM-3(보유 Permission만 매핑 가능)이 담당. `is_system` 역할은 삭제·코드 변경 불가. 역할 속성(`requires_2fa` 포함) 변경도 본 권한으로 통제하고 감사 기록한다(§10.4)
...
`users.must_change_password` 또는 `users.totp_enrollment_required` 플래그가 켜진 계정은 로그인(AUTH-2)은 가능하되(`status=ACTIVE` 유지), 발급되는 세션의 접근 범위가 **온보딩 API(비밀번호 변경, TOTP 등록)로만 제한**된다. 두 플래그를 모두 해소해야 일반·관리 API 접근이 열린다.

- **플래그 설정 시점**: (a) 최초 `SUPER_ADMIN` 시드 생성 시 두 플래그 모두 설정(§13.1). (b) `requires_2fa=true` 역할을 TOTP 미등록자에게 부여할 때 `totp_enrollment_required` 설정 — 이때 대상자의 `perm_version` 증가와 리프레시 토큰 패밀리 전체 폐기를 동반하여 재로그인을 강제한다(기존 세션·갱신 경로 양쪽 차단).
- **플래그 해소 시점**: 각각 비밀번호 변경 완료, TOTP 등록 완료 시 해제.
- **방어 심층**: 토큰 갱신(AUTH-3) 시점에도 `requires_2fa` 역할 보유+TOTP 미등록이면 갱신을 거부한다.
- 상태(`status`)와 온보딩은 분리된 개념이다 — 온보딩 미완료는 정지(SUSPENDED)가 아니며, 별도의 상태 값을 만들지 않는다(로그인 거부 규칙과의 순환 방지).
...
**쿼럼 승인 구조 (`SUPER_ADMIN` 비상 회수 — 예약)**: 본 워크플로의 선행 구현으로, `SUPER_ADMIN`을 대상으로 하는 관리 조치(정지·역할 회수)에 한해 우위 검사(§4.6-1)의 동집합 거부에 대한 **의도적·명시적 예외 경로**를 둔다. 발의자 외 타 활성 `SUPER_ADMIN` 과반수(최소 2인) 승인이 모이면 조치가 실행되며, 발의·승인·실행 전 과정을 감사 로그에 별도 심각도로 기록한다. 쿼럼은 발의자 외 활성 `SUPER_ADMIN`이 2인 이상, 즉 **전체 활성 `SUPER_ADMIN`이 3인 이상일 때만 성립**한다 — 2인 이하 체제에서는 쿼럼 경로가 원천 불성립하며 break-glass 절차(§10.1)만 유효하다. 따라서 이 구조 도입 후에도 break-glass는 최후 수단으로 유지한다. 발주자 결정(2026-08-07)에 따라 Phase 3 확장으로 예약하며, Phase 1은 break-glass 절차만으로 대응한다.
...
- **`requires_2fa=true` 역할(§5.2 roles)을 하나라도 보유한 계정은 2FA(TOTP)를 강제한다.** 시드 기준 `OPERATOR`·`SUPER_ADMIN`이 해당한다. "`OPERATOR` 이상" 같은 서수 표현은 rank 제거(v1.2, INV-2) 이후 판정 근거가 없으므로 사용하지 않는다 — 판정은 역할 데이터에서 파생되며, 신규 역할 생성 시 이 플래그를 명시적으로 선택하므로 조용한 2FA 누락이 발생하지 않는다. 플래그 변경은 ADM-2 경로(`admin.role.manage`)로만 가능하며 감사 기록한다.
...
3. 역할 5종 + display_order + `requires_2fa`(`OPERATOR`·`SUPER_ADMIN` true) + 매핑 (§4.5)
4. 최초 `SUPER_ADMIN` 계정: 환경 변수 기반 생성 스크립트(비밀번호 하드코딩 금지), `status=ACTIVE` + 온보딩 플래그(`must_change_password`·`totp_enrollment_required`) 설정 — 온보딩 게이트(§8.5)에 의해 비밀번호 변경·TOTP 등록 완료 전에는 온보딩 API 외 접근 불가
...
1. **권한 모델의 변경 = 마이그레이션**: permissions·roles·매핑의 모든 변경은 forward-only 마이그레이션 파일로만 수행한다. 운영 DB 직접 수정 금지 — **유일한 예외는 §10.1의 break-glass 절차**(2인 입회·전량 감사·사후 보고를 전제로 한 비상 회수)이며, 그 외의 직접 수정은 어떤 사유로도 허용하지 않는다. 마이그레이션 이력이 곧 권한 모델의 버전 이력이 된다.
...
### v1.6 (2026-08-07) — 2FA 강제 기준의 데이터 파생화 · 온보딩 게이트 명세

3차 재검수 보고서(`phase1-work-order-review-v3.md` RT-11·RT-12) 반영.

1. **`roles.requires_2fa` 도입 (RT-11)**: §10.4의 "`OPERATOR` 이상"은 rank 제거(v1.2) 이후 판정 근거가 없는 잔재였다(레드팀이 자기 지적으로 확인). 2FA 강제를 "`requires_2fa=true` 역할 보유"로 재정의 — 판정이 데이터에서 파생되고(INV-2 정합), 신규 역할의 조용한 2FA 누락이 구조적으로 차단된다. §5.2 DDL·§4.5 시드·ADM-2 통제 경로 반영.
2. **온보딩 게이트 명세화 (RT-12)**: 작업지시서 v3(RT-8 해소)에서 도입된 `must_change_password`·`totp_enrollment_required` 플래그와 `totp_secret`(암호화 보관)을 §5.2 `users` DDL에 편입하고, §8.5를 신설하여 게이트 규칙(접근 범위 제한, 설정·해소 시점, 갱신 시 방어 심층, 상태와의 분리)에 기획서상 근거를 부여 — "충돌 시 기획서 우선" 원칙과의 공백 해소. §13.1 시드 체크리스트 갱신.

### v1.5 (2026-08-07) — 재검수 정합성 반영

레드팀 재검수 보고서(`phase1-work-order-review-v2.md` RT-10-2·RT-10-3) 반영.

1. **§16.3-1에 break-glass 상호 참조 추가**: "운영 DB 직접 수정 금지"의 유일한 예외가 §10.1 break-glass 절차임을 명기 — v1.4에서 두 조항이 모순으로 읽히던 문제 해소 (RT-10-2).
2. **§9.5 쿼럼 성립 조건 명시**: 쿼럼은 전체 활성 `SUPER_ADMIN` 3인 이상일 때만 성립하며, 2인 이하 체제에서는 break-glass만 유효함을 명기 (RT-10-3).
