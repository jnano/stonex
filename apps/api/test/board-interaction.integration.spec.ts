...
describe('게시판 상호작용 (WP-B3, 실 DB)', () => {
  ...
  beforeAll(async () => {
    ...
    notifications = new BoardNotificationService(p, policy, new PostPolicyService(p, new PrismaGrantStore(p)), capabilities);
    bus = new BoardEventBus(p);
    bus.register(notifications);
    const storage = new StorageService(new SettingsService(p, new AuditService()));
    const attachments = new BoardAttachmentService(p, audit, storage, new UploadSessionService(p, storage), boards, capabilities);
    ...
    comments = new CommentsService(p, audit, boards, bus, new PostPolicyService(p, new PrismaGrantStore(p)), new CommentReactionsService(p, capabilities), capabilities);
