...
-- 감사 로그: 월 단위 파티셔닝 (기획서 §5.2, R-1 — Prisma 모델 없음, raw SQL 전용)
...
-- 파티션 생성 함수: 지정 월의 파티션을 만든다(존재 시 무시).
-- 운영 절차: 매월 배치(또는 WP-6b 스크립트)가 다음 달 파티션을 선생성한다.
...
-- 초기 파티션: 당월 + 익월
SELECT create_audit_log_partition(now()::date);
SELECT create_audit_log_partition((now() + interval '1 month')::date);
